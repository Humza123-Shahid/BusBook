...
const EditFaqs = () => {
    ...
    const {editFaqs}=context;
    const [showToast,setShowToast]=useState(false)
        const [msg,setMsg]=useState('')
        const [type,setType]=useState('')
    const location = useLocation();
    const Faqs=location.state?.faqs || {};
    const [question, setQuestion] = useState(Faqs.question);
    const [answer, setAnswer] = useState(Faqs.answer);
    const [selectedStatusValue, setSelectedStatusValue] = useState(Faqs.status);
  ...
  const editFaq=(e)=>{
          e.preventDefault();
          const success=editFaqs(Faqs._id,question,answer,selectedStatusValue)
          if(success)
          {
            setShowToast(true);
            setMsg("Faqs updated successfully")
            setType("success")
            setTimeout(()=>{
              setShowToast(false)
            },1500)
  ...
  return (
    ...
    <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
     ...
     {/* <div className="mb-3 ms-3" style={{width:'100%'}}>
      ...
      <button disabled={question.length<1||answer.length<1} type="submit" className="btn btn-primary" >Edit Faqs</button>
      </form>
    </div>
  )
}
