...
const AdminDestination = () => {
    ...
    const {destinations,deleteDestination,getDestinations}=context;
  ...
  //   }
  // }, []);
  const filteredData = Array.isArray(destinations) && destinations.filter(item =>
      item.name?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  const handleView = (id,index) => {
    const dataitem=destinations.find(da => da._id ==id)
    navigate('getdestination', { state: { destination:dataitem,idx:index} });
     
  };
  const handleEdit = (id) => {
    const dataitem=destinations.find(da => da._id ==id)
    navigate('editdestination', { state: { destination:dataitem} });
  };
  ...
  return (
   <div>
      <button className="btn btn-primary mt-3 ms-4" onClick={handleClick}>Add Destination</button>
      <div className="container d-flex justify-content-between">
        <h3 className="ms-2">Destination Data</h3>
