...
const AdminExpenceCategories = () => {
     const context=useContext(expencecategoryContext);
      const {expenceCategories,getExpenceCategories,deleteExpenceCategory}=context;
    const navigate = useNavigate();
    const [searchTerm, setSearchTerm] = useState('');
    const handleClick = () => {
        navigate('addexpencecategory');
  ...
  //   }
  // }, []);
  const filteredData = expenceCategories.filter(item =>
      item.name?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  const handleView = (id,index) => {
    const dataitem=expenceCategories.find(da => da._id ==id)
    navigate('getexpencecategory', { state: { expencecategory:dataitem,idx:index} });
     
  };
  const handleEdit = (id) => {
    const dataitem=expenceCategories.find(da => da._id ==id)
    navigate('editexpencecategory', { state: { expencecategory:dataitem} });
  };
  const handleDelete = (id) => {
     const confirmed = window.confirm("Are you sure you want to delete this?");
  if (confirmed) {
    deleteExpenceCategory(id);
  ...
  return (
   <div>
      <button className="btn btn-primary mt-3 ms-4" onClick={handleClick}>Add Expence Category</button>
      <div className="container d-flex justify-content-between">
        <h3 className="ms-2">Expence Categories Data</h3>
...
export default AdminExpenceCategories
